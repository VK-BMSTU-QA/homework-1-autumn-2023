...
- Поле ввода почты:
  ...
  - ${\color{red}при \ длине \ почты \ более \ 250 \ символов \ показывает \ Internal \ Server \ Error \ в \ форме, \ без \ пояснений}$
...
- ${\color{red}При \ наличии \ ошибки \ у \ какого-либо \ из \ полей \ формы, \ при \ исправлении \ этой \ ошибки \ и \ отправке \ формы \ с \ иной \ ошибкой, \ старая \ ошибка \ всё \ так \ же \ отображается}$
...
- Для авторизованного пользователя:
  - ${\color{red}кнопка \ выхода \ из \ профиля \ не \ реагирует \ на \ наведение \ мыши}$
  ...
  - клик на аватарку или никнейм редиректит на страницу профиля
...
- При нажатии на название города, открывается модальное окно выбора города:
  - город можно найти по подстроке, однако ${\color{red}строка \ поиска \ учитывает \ регистр}$
...
- Для неавторизованного пользователя:
  ...
  - ${\color{red}При \ клике \ на \ кнопку \ приглашения \ происходит \ редирект \ на \ регистрацию, \ а \ не \ на \ логин}$
...
- Для авторизованного пользователя:
  ...
  - При клике на кнопку приглашения:
    - ${\color{red}При \ отсутствии \ подписок \ видим \ пустое \ модальное \ окно}$  
  ...
  - При клике на кнопку добавления в запланированные мероприятия:
    ...
    - ${\color{red}никаких \ уведомлений \ о \ предстоящем \ мероприятии \ не \ приходит: \ ни \ на \ почту, \ ни \ в \ уведомлениях \ сайта}$
...
- При нажатии на логотип Telegram открывается t.me/share.  
  ${\color{red} Если \ не \ установлено \ десктопное \ приложение \ Telegram, \ кнопка \ share \ ничего \ не \ делает.}$  
  Если поставить приложение, браузер предлагает открыть его. В приложении удается поделиться ссылкой
- ${\color{red} Ссылка \ на \ мероприятие, \ которой \ делимся \ в \ соцсетях, \ отображается \ без \ какого-либо \ оформления.}$ При нажатии на ссылку открывается ожидаемая страница мероприятия
...
- ${\color{red} Окно \ закрывается \ при \ перезагрузке \ страницы}$
...
- ${\color{red}При \ нажатии \ на \ кнопку \ приглашения \ не \ происходит \ отклика \ для \ пользователя \ - непонятно, \ отправилось \ ли \ приглашение.}$ Окно остается открытым
...
- ${\color{red}Приглашение \ не \ приходит \ на \ почту, \ указанную \ при \ регистрации}$
- При нажатии на колокольчик открывается модальное окно с уведомлениями
- При принятии/отклонении приглашения в одной вкладке браузера, приглашении все ёще существует в других вкладках. ${\color{red}Попытка \ повторно \ принять/отклонить \ не \ даёт \ никакой \ реакции, \ в \ консоли \ 404 \ из-за \ отсутствия \ приглашения.}$
